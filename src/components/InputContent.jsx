...
export default function InputContent({ className, contents, clicked, setResult, inputValue, setInputValue, selectedFrom, setSelectedFrom, selectedTo, setSelectedTo, setWeight, setHeight, weight, height, unitsFrom, setUnitsFrom, unitsTo, setUnitsTo, result, setFinalInputValue, setFinalSecondInput, setBmi}) {
  ...
  const units = contents && contents.filter(obj => obj[clicked]).map((obj) => obj[clicked][1].units).flat()
    ...
    <div className={className}>
      <div className="max-[570px]:justify-center gap-2 flex max-[640px]:flex-col-reverse w-full">
        ...
        <div className="w-full flex gap-2">
          ...
          <input type="button" value="Convert" className="w-full max-[570px]:w-full h-12 rounded-lg bg-gray-800 cursor-pointer hover:bg-slate-700 transition-all duration-150" onClick={() => (inputValue || weight || height) && Convert(units, clicked, inputValue, weight, height, selectedFrom, selectedTo, setResult, setFinalInputValue, setFinalSecondInput, setInputValue, setWeight, setHeight, setBmi)}/>
        </div>
      </div>
    </div>
  ) 
